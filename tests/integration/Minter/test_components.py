...
def minter_setup(accounts, coin_deposit, mock_lp_token, gauge_controller, gauge_v3):
    # set type
    gauge_controller.add_type(b"Liquidity", 10 ** 18, {"from": accounts[0]})

    # add gauge
    gauge_controller.add_gauge(gauge_v3, 0, 10 ** 19, {"from": accounts[0]})

    # transfer tokens
    for acct in accounts[0:4]:
        coin_deposit.mint(acct, 1e18, {"from": accounts[0]})
        coin_deposit.approve(mock_lp_token, 1e18, {"from": acct})


@given(st_duration=strategy("uint[3]", min_value=WEEK, max_value=MONTH, unique=True))
@settings(max_examples=30)
def test_duration(accounts, chain, gauge_v3, coin_deposit, mock_lp_token, minter, token, st_duration):
    accts = accounts[1:]
    chain.sleep(7 * 86400)

    deposit_time = []
    for i in range(3):
        mock_lp_token.deposit(10 ** 18, {"from": accts[i]})
    ...
    for i in range(3):
        ...
        mock_lp_token.withdraw(10 ** 18, {"from": accts[i]})
        durations.append(chain[-1].timestamp - deposit_time[i])
        minter.mint(gauge_v3, {"from": accts[i]})
...
@settings(max_examples=30)
def test_amounts(accounts, chain, gauge_v3, coin_deposit, mock_lp_token, minter, token, st_amounts):
    accts = accounts[1:]

    deposit_time = []
    for i in range(3):
        mock_lp_token.deposit(st_amounts[i], {"from": accts[i]})
        deposit_time.append(chain[-1].timestamp)

    chain.sleep(MONTH)
    balances = []
    for i in range(3):
        mock_lp_token.withdraw(st_amounts[i], {"from": accts[i]})
