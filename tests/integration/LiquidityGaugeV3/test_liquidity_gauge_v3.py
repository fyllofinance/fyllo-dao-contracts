...
def test_gauge_integral(accounts, chain, coin_deposit, mock_lp_token, token, gauge_v3, gauge_controller, reward_policy_maker):
    ...
    coin_deposit.mint(bob, 10 ** 21, {"from": alice})
    coin_deposit.mint(alice, 10 ** 21, {"from": alice})

    def update_integral():
        nonlocal checkpoint, integral, checkpoint_balance, checkpoint_supply
        ...
        if checkpoint >= t_epoch:
            ...
            prev_time = checkpoint
            rate_x_time = 0
            for i in range(500):
                week_time = reward_policy_maker.epoch_start_time(reward_policy_maker.epoch_at(prev_time + WEEK))
                week_time = min(week_time, t1)
                rate = reward_policy_maker.rate_at(prev_time)
                rate_x_time += (week_time - prev_time) * rate
                if week_time == t1:
                    break
                prev_time = week_time
        ...
        checkpoint = t1
        checkpoint_supply = gauge_v3.working_supply()
        checkpoint_balance = gauge_v3.working_balances(alice)
    ...
    for i in range(40):
        ...
        if is_withdraw:
            ...
            mock_lp_token.withdraw(amount, {"from": bob})
            update_integral()
            bob_staked -= amount
        else:
            amount = randrange(1, coin_deposit.balanceOf(bob) // 10 + 1)
            coin_deposit.approve(mock_lp_token.address, amount, {"from": bob})
            mock_lp_token.deposit(amount, {"from": bob})
            update_integral()
            bob_staked += amount
        ...
        if is_alice:
            ...
            if is_withdraw_alice:
                ...
                mock_lp_token.withdraw(amount_alice, {"from": alice})
                update_integral()
                alice_staked -= amount_alice
            else:
                amount_alice = randrange(1, coin_deposit.balanceOf(alice) + 1)
                coin_deposit.approve(mock_lp_token.address, amount_alice, {"from": alice})
                mock_lp_token.deposit(amount_alice, {"from": alice})
                update_integral()
                alice_staked += amount_alice
...
def test_mining_with_votelock(
    ...
    coin_deposit.mint(alice, 10 ** 21, {"from": alice})
    coin_deposit.mint(bob, 10 ** 21, {"from": alice})
    coin_deposit.approve(mock_lp_token.address, MAX_UINT256, {"from": alice})
    coin_deposit.approve(mock_lp_token.address, MAX_UINT256, {"from": bob})
    ...
    mock_lp_token.deposit(10 ** 21, {"from": bob})
