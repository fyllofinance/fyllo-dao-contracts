...
class StateMachine:
    ...
    def __init__(self, LiquidityGaugeV3, accounts, gauge_controller, mock_lp_token, minter, reward_policy_maker):
        self.LiquidityGaugeV3 = LiquidityGaugeV3
    ...
    def rule_add_gauge(self, st_type, st_gauge_weight):
        ...
        gauge_name = self.lp_token.symbol() + "-gauge"
        gauge = self.LiquidityGaugeV3.deploy(
            gauge_name, self.lp_token, self.minter, self.accounts[0], self.reward_policy_maker,
            0, 0, {"from": self.accounts[0]}
        )
...
def test_gauge(state_machine, LiquidityGaugeV3, accounts, gauge_controller, mock_lp_token, minter, reward_policy_maker):
    state_machine(StateMachine, LiquidityGaugeV3, accounts, gauge_controller, mock_lp_token, minter, reward_policy_maker)
